...
export function validateVisualValidationTeaching(
  ...
  if (input.entryBufferTicks !== 4) messages.push("The entry buffer must be exactly four MES ticks (1.00 index point).");
...
export function buildProposedRuleAnalysis(
  ...
  for (const review of teachingReviews) {
    ...
    if (review.teaching?.entryBufferTicks !== 4) likelyCauses.add("Reviewers are testing a non-governed confirmation buffer against the active four-tick formula.");
