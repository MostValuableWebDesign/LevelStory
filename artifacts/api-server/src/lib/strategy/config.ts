...
export function validateStrategyConfig(config: StrategyConfig): StrategyConfig {
  ...
  if (config.profitTargetPlacement !== "NEAR_SIDE_20_TICKS" && config.profitTargetPlacement !== "EXACT_LEVEL") {
    throw new Error("Invalid strategy configuration: profitTargetPlacement must be NEAR_SIDE_20_TICKS or EXACT_LEVEL.");
  }
