...
test("early ORB momentum uses a qualifying patience-shaped outside close as P and the adjacent candle as the only E", () => {
...
test("Early ORB rejects the raw 9:55 breakout and lets the later 10:05 patience candle confirm", () => {
  const result = earlyOrbMomentumPatienceAnalysis([
    datedCandle("2026-02-26T14:30:00.000Z", 6950, 6960, 6940, 6955),
    datedCandle("2026-02-26T14:35:00.000Z", 6955, 6961, 6945, 6958),
    datedCandle("2026-02-26T14:40:00.000Z", 6958, 6960, 6936.75, 6948),
    datedCandle("2026-02-26T14:45:00.000Z", 6948, 6950, 6940, 6945),
    datedCandle("2026-02-26T14:50:00.000Z", 6945, 6950, 6933.5, 6938),
    // This closes below the ORB, but its low breaks the prior low, so it is
    // not a valid short patience candle.
    datedCandle("2026-02-26T14:55:00.000Z", 6938, 6940, 6931.75, 6935.5),
    datedCandle("2026-02-26T15:00:00.000Z", 6935.5, 6938, 6923.25, 6930),
    // The intended P holds above the prior low while closing outside the ORB.
    datedCandle("2026-02-26T15:05:00.000Z", 6930, 6932, 6926.25, 6927.25),
    datedCandle("2026-02-26T15:10:00.000Z", 6927.25, 6930, 6913, 6920),
  ], {
    high: 6961,
    low: 6936.75,
    complete: true,
  ...
  assert.equal(result.direction, "short");
  assert.equal(result.state, "ENTRY_TRIGGERED");
  assert.equal(result.patienceCandle?.openTime, Date.parse("2026-02-26T15:05:00.000Z"));
  assert.equal(result.triggerCandle?.openTime, Date.parse("2026-02-26T15:10:00.000Z"));
  assert.equal(result.occurrences?.some((occurrence) => occurrence.patienceCandle.openTime === Date.parse("2026-02-26T14:55:00.000Z")), false);
  assert.equal(result.occurrences?.filter((occurrence) => occurrence.direction === "short").length, 1);
});
...
test("patience strategy stops use exactly twelve MES ticks beyond the frozen P extreme", () => {
  ...
  assert.equal(long.strategyStopPrice, 4);
  assert.equal(short.state, "ENTRY_TRIGGERED");
  assert.equal(short.strategyStopPrice, 16);
  assert.equal(authoritativePatienceStopPrice("long", 6975.75), 6972.75);
...
test("the seventh tick does not confirm, while the eighth tick confirms", () => {
  ...
  assert.equal(eightTicks.strategyStopPrice, 4);
});
...
test("configured confirmation and stop buffers are retained on every patience occurrence", () => {
  ...
  assert.equal(result.occurrences?.[0]?.stopBufferTicks, 12);
  assert.equal(result.occurrences?.[0]?.patienceCandleExtreme, result.occurrences?.[0]?.patienceCandle.low);
  assert.equal(result.occurrences?.[0]?.stopBufferPoints, 3);
  assert.equal(result.occurrences?.[0]?.finalStopBoundary, 4);
});
...
test("eight-tick confirmation is governed and the thesis stop sits twelve ticks beyond the opposite wick", () => {
  ...
  assert.equal(result.strategyStopPrice, 4);
});
