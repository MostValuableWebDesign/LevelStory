         ...
         <div className="control-ribbon mb-5 flex flex-col gap-3 p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
           <div className="flex flex-col gap-3 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><span className="eyebrow px-1 text-muted-foreground">Watchlist</span><div className="flex flex-wrap gap-1">{symbols.map((item) => <button type="button" key={item} onClick={() => setSymbol(item)} className={`rounded-sm px-3 py-2 text-xs font-bold transition-colors ${symbol === item ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`} aria-pressed={symbol === item} data-testid={`button-symbol-${item.toLowerCase()}`}>{item}</button>)}</div></div><div className="flex rounded-sm border border-border p-0.5"><button type="button" onClick={() => setSession("premarket")} className={`px-2.5 py-1.5 text-[10px] font-bold uppercase ${session === "premarket" ? "bg-secondary text-foreground" : "text-muted-foreground"}`} aria-pressed={session === "premarket"} data-testid="button-session-premarket">Premarket</button><button type="button" onClick={() => setSession("regular")} className={`px-2.5 py-1.5 text-[10px] font-bold uppercase ${session === "regular" ? "bg-secondary text-foreground" : "text-muted-foreground"}`} aria-pressed={session === "regular"} data-testid="button-session-regular">Replay</button></div></div>
              <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
               ...
               {targetSelection === "custom" && <input aria-label="Custom target dollars" type="number" min="50" max="100" step="0.01" value={customTarget} onChange={(event) => setCustomTarget(event.target.value)} className="field mono h-9 w-24 px-2 text-xs" data-testid="input-custom-target-dollars" />}
           ...
           <div className="grid gap-5 lg:grid-cols-[.95fr_1.05fr]">
             <Panel>
               <PanelTitle eyebrow="Risk / no execution" title="Position plan" right={<span className={`text-[10px] font-bold uppercase ${snapshot.riskPlan.allowed ? "status-positive" : "status-negative"}`}>{snapshot.riskPlan.allowed ? "Allowed" : "Blocked"}</span>} />
                <div className="grid grid-cols-2 gap-px border-t border-border bg-border sm:grid-cols-4">{[["Direction", snapshot.riskPlan.direction], ["Entry", formatPrice(snapshot.riskPlan.entry)], ["Strategy stop", formatPrice(snapshot.riskPlan.strategyStop)], ["Catastrophe stop", formatPrice(snapshot.riskPlan.catastropheStop)], ["Target", `${formatPrice(snapshot.riskPlan.target)} · $${snapshot.riskPlan.targetDollars}`], ["Contracts", `${snapshot.riskPlan.contracts} (${snapshot.riskPlan.targetContracts} target / ${snapshot.riskPlan.runnerContracts} runner)`], ["Risk / contract", `$${snapshot.riskPlan.riskPerContract.toFixed(2)}`], ["Total risk", `$${snapshot.riskPlan.dollarRisk.toFixed(2)}`]].map(([label, value]) => <div key={label} className="bg-card px-4 py-4"><div className="text-[10px] text-muted-foreground">{label}</div><div className="mono mt-1 text-sm font-medium">{value}</div></div>)}</div>
                <div className="grid gap-3 border-t border-border p-5 sm:grid-cols-2"><div><div className="eyebrow text-muted-foreground">Cost model · {snapshot.riskPlan.slippageMode}</div><p className="mt-2 text-xs text-muted-foreground">Slippage ${snapshot.riskPlan.costBreakdown.totalSlippage.toFixed(2)} · fees ${snapshot.riskPlan.costBreakdown.roundTripFees.toFixed(2)}</p><p className="mt-1 text-[10px] text-muted-foreground">Commission ${snapshot.riskPlan.costBreakdown.commission.toFixed(2)} · exchange ${snapshot.riskPlan.costBreakdown.exchange.toFixed(2)} · regulatory ${snapshot.riskPlan.costBreakdown.regulatory.toFixed(2)} · clearing ${snapshot.riskPlan.costBreakdown.clearing.toFixed(2)}</p></div><div><div className="eyebrow text-muted-foreground">Projected target leg</div><p className={`mono mt-2 text-sm font-medium ${snapshot.riskPlan.projectedTargetPnl.netPnl >= 0 ? "status-positive" : "status-negative"}`}>Gross ${snapshot.riskPlan.projectedTargetPnl.grossPnl.toFixed(2)} · net ${snapshot.riskPlan.projectedTargetPnl.netPnl.toFixed(2)}</p><p className="mt-1 text-[10px] text-muted-foreground">Runner: {snapshot.riskPlan.runner.active ? `active · retracement ${snapshot.riskPlan.runner.adverseRetracement.toFixed(2)} / ${snapshot.riskPlan.runner.retracementThreshold?.toFixed(2) ?? "—"}` : snapshot.riskPlan.runnerContracts ? "activates after target leg" : "none designated"}</p></div></div>
                <div className="space-y-2 border-t border-border p-5">{snapshot.riskPlan.reasons.map(reason => <p key={reason} className="text-xs leading-5 text-muted-foreground">{reason}</p>)}<p className="text-[10px] uppercase tracking-[.08em] text-muted-foreground">Safety locks: {Object.entries(snapshot.riskPlan.locks).map(([key, value]) => `${key} ${value ? "ON" : "clear"}`).join(" · ")}</p></div>
             </Panel>
             <Panel>
  ...
  return <div className="space-y-5">
    <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
      <Panel accent={breakout.detected}>
        <PanelTitle eyebrow="Phase 4 / ORB quality lifecycle" title="ORB breakout" right={<span className={`text-[10px] font-bold uppercase ${breakout.detected ? "status-positive" : breakout.failed ? "status-negative" : "text-muted-foreground"}`}>{breakout.state.replaceAll("_", " ")}</span>} />
        <div className="grid grid-cols-2 gap-px border-t border-border bg-border sm:grid-cols-4">
          {[["Qualified close", breakout.time ? new Date(breakout.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "—"], ["Candidate", breakout.candidateTime ? new Date(breakout.candidateTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "—"], ["Distance", breakout.distanceOutside == null ? "—" : `${breakout.distanceOutside.toFixed(2)} / ${breakout.meaningfulDistance?.toFixed(2) ?? "—"} pts`], ["Volume", breakout.volumeRatio == null ? "—" : `${breakout.volumeRatio.toFixed(2)}×`]].map(([label, value]) => <div key={label} className="bg-card px-4 py-4"><div className="text-[10px] text-muted-foreground">{label}</div><div className="mono mt-1 text-sm font-medium">{value}</div></div>)}
        </div>
        <div className="grid grid-cols-2 gap-px border-t border-border bg-border sm:grid-cols-4">
          {[["Body", breakout.bodyRatio == null ? "—" : `${(breakout.bodyRatio * 100).toFixed(0)}%`], ["Close location", breakout.closeLocationRatio == null ? "—" : `${(breakout.closeLocationRatio * 100).toFixed(0)}%`], ["Structure", breakout.candleStructureSupported ? "Passed" : "Waiting"], ["Continuation", breakout.continuationCondition?.replaceAll("_", " ") ?? "Waiting"]].map(([label, value]) => <div key={label} className="bg-card px-4 py-3"><div className="text-[10px] text-muted-foreground">{label}</div><div className="mono mt-1 text-[11px] font-medium uppercase">{value}</div></div>)}
        </div>
