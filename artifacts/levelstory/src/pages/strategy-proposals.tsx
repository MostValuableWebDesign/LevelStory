...
export default function StrategyProposals() {
  ...
  useEffect(() => {
    if (!selectedId || !detail || !["validation_pending", "validation_running"].includes(detail.proposal.status)) return;
    const timer = window.setInterval(() => {
      void api<Detail>(`/strategy-proposals/${selectedId}`).then(setDetail).catch(() => undefined);
    }, 750);
    return () => window.clearInterval(timer);
  }, [selectedId, detail?.proposal.status]);
  ...
  const create = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const proposal = await api<Proposal>("/strategy-proposals", {
       ...
       body: JSON.stringify({ title: String(form.get("title")), hypothesis: String(form.get("hypothesis")), rationale: String(form.get("rationale")), sourceTeachingIds, proposalPayload: { mode: "shadow_only", source: "human_teaching", ruleDiff: [{ field: String(form.get("ruleField") || "patienceEntryBufferTicks"), value: Number(form.get("ruleValue") || 4) }] } }),
      });
      setShowCreate(false);
...
function ProposalDetail({ detail, latestRun, onAction, teachings }: { detail: Detail; latestRun?: ValidationRun; onAction: (action: string, reason?: string) => void; teachings: TeachingExample[] }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
       <Panel>
         <PanelTitle eyebrow="Validation / durable comparison" title="Evidence gate" right={latestRun ? <StatusPill status={latestRun.status} /> : <Clock3 size={15} className="text-muted-foreground" />} />
         {latestRun ? <div className="space-y-3 border-t border-border px-5 py-4 text-xs sm:px-6">
           <div className="flex items-center gap-2"><ValidationIcon status={latestRun.status} /><span>{latestRun.status === "queued" || latestRun.status === "running" ? `Validation is still running: ${latestRun.progressStage ?? "queued"} (${latestRun.progressPercent ?? 0}%). Approval remains blocked.` : latestRun.status === "passed" ? "Parent and candidate comparison passed all gates. An approver may review the evidence." : latestRun.errorMessage || "Validation failed. Resolve the evidence conflict before approval."}</span></div>
           {latestRun.afterMetrics && latestRun.afterMetrics.performanceMetricsAvailable === false && <div className="border border-accent/30 bg-accent/10 px-3 py-2 text-accent">Candidate execution replay has not been completed. Performance metrics are Not evaluated and governed transitions remain disabled.</div>}
           <div className="grid gap-2 sm:grid-cols-2"><MetricLine label="Warnings" value={latestRun.warnings.length} /><MetricLine label="Conflicts" value={latestRun.conflicts.length} /><MetricLine label="Regressions" value={latestRun.regressions.length} /><MetricLine label="Before sample" value={metricNumber(latestRun.beforeMetrics, "sampleCount")} /><MetricLine label="After sample" value={metricNumber(latestRun.afterMetrics, "sampleCount")} /><MetricLine label="Trades added" value={metricNumber(latestRun.afterMetrics, "tradesAdded")} /><MetricLine label="Trades removed" value={metricNumber(latestRun.afterMetrics, "tradesRemoved")} /><MetricLine label="Win rate" value={metricNumber(latestRun.afterMetrics, "winRate")} /><MetricLine label="Net P&L" value={metricNumber(latestRun.afterMetrics, "netPnl")} /><MetricLine label="Profit factor" value={metricNumber(latestRun.afterMetrics, "profitFactor")} /><MetricLine label="Max drawdown" value={metricNumber(latestRun.afterMetrics, "maximumDrawdown")} /><MetricLine label="Holdout count" value={metricNumber(latestRun.afterMetrics, "holdoutCount")} /></div>
           {[...latestRun.conflicts, ...latestRun.regressions, ...latestRun.warnings].map((item) => <div key={item} className="border-l-2 border-accent/50 pl-3 text-muted-foreground">{item}</div>)}
         </div> : <div className="border-t border-border px-5 py-6 text-xs leading-5 text-muted-foreground sm:px-6">No validation run exists. Validation is required before approval and does not alter the active formula.</div>}
       </Panel>
...
function metricNumber(metrics: Record<string, unknown> | null | undefined, key: string): number | string {
  if (metrics?.performanceMetricsAvailable === false && ["winRate", "netPnl", "profitFactor", "maximumDrawdown", "completedTrades", "stopOutcomes", "targetOutcomes", "runnerOutcomes"].includes(key)) return "Not evaluated";
