...
import { BookOpen, Check, CircleAlert, Plus, Trash2 } from "lucide-react";
...
export default function Journal() {
  ...
  const deleteEntry = (id: number) => {
    ...
    remove.mutate({ id }, { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListJournalEntriesQueryKey({ limit: 50 }) }); queryClient.invalidateQueries({ queryKey: getGetDashboardOverviewQueryKey() }); } });
  };

  return <LevelStoryShell><div className="cockpit-grid min-h-[calc(100dvh-68px)] px-5 py-7 sm:px-8 lg:px-10 lg:py-9"><div className="mx-auto max-w-[1480px]">
    ...
    <div className="grid gap-5 xl:grid-cols-[.72fr_1.28fr]">
      <Panel accent>
        ...
        <form onSubmit={submit} className="space-y-4 border-t border-border p-5 sm:p-6">
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-muted/35 p-3"><input type="checkbox" checked={form.checklistPassed} onChange={(e) => update("checklistPassed", e.target.checked)} className="mt-0.5 accent-[hsl(var(--accent))]" data-testid="checkbox-journal-checklist" /><span><span className="block text-xs font-semibold">Checklist passed</span><span className="mt-1 block text-[11px] leading-4 text-muted-foreground">I followed the plan before reviewing this trade.</span></span></label>
          {message && <div className={`flex items-center gap-2 rounded-lg p-3 text-xs ${message.includes("couldn't") ? "bg-destructive/10 text-destructive" : "bg-accent/15 text-foreground"}`} data-testid="status-journal-message">{message.includes("couldn't") ? <CircleAlert size={14} /> : <Check size={14} />}{message}</div>}
          <button type="submit" disabled={create.isPending} className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-xs font-bold text-primary-foreground transition hover:opacity-90 disabled:opacity-50" data-testid="button-save-journal"><Plus size={15} />{create.isPending ? "Recording review..." : "Record shadow review"}</button>
          <LockedNote>This is a journal action, not an order action. Nothing is sent to a broker.</LockedNote>
        </form>
      </Panel>
      <Panel>
        <PanelTitle eyebrow="Your record" title="Recent shadow reviews" right={<span className="mono text-xs text-muted-foreground">{entries.data?.length ?? 0} entries</span>} />
        {entries.isLoading ? <QuerySkeleton rows={5} /> : entries.isError ? <QueryError onRetry={() => entries.refetch()} /> : entries.data?.length === 0 ? <EmptyJournal /> : <div className="divide-y divide-border border-t border-border">{entries.data?.map((entry) => <article key={entry.id} className="group px-5 py-5 sm:px-6" data-testid={`card-journal-entry-${entry.id}`}><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-secondary text-xs font-bold">{entry.symbol.slice(0, 2)}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-bold">{entry.symbol}</h3><span className="rounded bg-secondary px-2 py-1 text-[10px] font-bold uppercase">{entry.side}</span>{entry.checklistPassed && <span className="inline-flex items-center gap-1 text-[10px] font-bold text-foreground"><Check size={12} />Plan held</span>}</div><p className="mt-1 text-xs text-muted-foreground">{entry.setup} · {new Date(entry.createdAt).toLocaleDateString()}</p></div></div><div className="flex items-center gap-4 sm:text-right"><div><div className={`mono text-sm font-medium ${(entry.pnl ?? 0) >= 0 ? "text-foreground" : "text-destructive"}`}>{entry.pnl == null ? "—" : `${entry.pnl >= 0 ? "+" : ""}$${entry.pnl.toFixed(2)}`}</div><div className="text-[10px] text-muted-foreground">{entry.quantity} shares</div></div><button type="button" onClick={() => deleteEntry(entry.id)} disabled={remove.isPending} className="rounded-md p-2 text-muted-foreground opacity-100 transition hover:bg-destructive/10 hover:text-destructive sm:opacity-0 sm:group-hover:opacity-100" aria-label={`Delete ${entry.symbol} journal entry`} data-testid={`button-delete-journal-${entry.id}`}><Trash2 size={15} /></button></div></div><p className="mt-4 max-w-3xl text-xs leading-5 text-muted-foreground">{entry.notes}</p></article>)}</div>}
      </Panel>
    </div>
...
function EmptyJournal() { return <div className="flex min-h-[420px] flex-col items-center justify-center px-8 text-center" data-testid="empty-journal"><div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-accent/20 text-foreground"><BookOpen size={24} /></div><h3 className="display text-xl font-bold">A blank page is useful.</h3><p className="mt-2 max-w-xs text-sm leading-6 text-muted-foreground">Your first shadow review will give tomorrow's discipline something real to learn from.</p></div>; }