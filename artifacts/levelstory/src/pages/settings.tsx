...
export default function Settings() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); setSaved(false); update.mutate({ data: { accountSize: Number(form.accountSize), riskPercent: Number(form.riskPercent), maxDailyLoss: Number(form.maxDailyLoss) } }, { onSuccess: (data) => { queryClient.setQueryData(getGetRiskSettingsQueryKey(), data); setSaved(true); } }); };
  return <LevelStoryShell><div className="cockpit-grid min-h-[calc(100dvh-68px)] px-5 py-7 sm:px-8 lg:px-10 lg:py-9"><div className="mx-auto max-w-[1120px]">
    <PageIntro eyebrow="Rules of engagement / always yours" title="Protect the decision-maker." description="Set the limits that keep a simulated session useful. These guardrails are designed to make restraint visible, not to optimize execution." action={<ShadowBadge />} />
    {settings.isLoading ? <Panel><QuerySkeleton rows={4} /></Panel> : settings.isError || !settings.data ? <Panel><QueryError onRetry={() => settings.refetch()} /></Panel> : <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-lg bg-muted/60 p-3"><div className="eyebrow text-muted-foreground">{label}</div><div className="mono mt-2 text-lg font-medium">{value}</div></div>; }