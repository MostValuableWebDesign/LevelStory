...
function ReviewPanel({
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(refreshed) !== JSON.stringify(teaching.qualifyingLevels ?? [])) {
      setTeaching({ ...teaching, qualifyingLevels: refreshed, pullbackLevels: refreshed.map((item) => item.valueAtInteraction) });
    }
  }, [levelCandle?.openTime, levelCandle?.closeTime, selectedIndicator?.openTime, selectedIndicator?.vwap, selectedIndicator?.ema200]);
  return <Panel accent>
      ...
      <div className="grid gap-2">
       ...
       {needsTeaching && <div className="space-y-3 border border-accent/35 bg-accent/5 p-4" data-testid="teaching-example-form">
         ...
         {teaching && <div className="grid gap-3 sm:grid-cols-2">
           ...
           <fieldset className="sm:col-span-2"><legend className="eyebrow mb-1.5 block text-muted-foreground">Qualifying pullback levels</legend><div className="grid gap-2 sm:grid-cols-2">{availableLevels.map((level) => { const price = level.price; const selected = (teaching.qualifyingLevels ?? []).some((item) => item.levelId === level.id); const contained = containedLevels.includes(level); const levelType = level.id === "vwap" || level.id === "ema-200" ? "dynamic_indicator" as const : level.rangeLow !== null || level.rangeHigh !== null ? "level_range" as const : "fixed_level" as const; return <label key={`${level.id}-${level.price}`} className={`flex items-center gap-2 border px-3 py-2 text-[11px] transition ${selected ? "border-accent bg-accent/10" : contained ? "cursor-pointer border-border bg-card hover:bg-muted/40" : "cursor-not-allowed border-border bg-muted/30 opacity-50"}`}><input type="checkbox" checked={selected} disabled={!contained} onChange={(event) => { const nextLevels = event.target.checked ? [...teaching.pullbackLevels.filter((value) => value !== price), price] : teaching.pullbackLevels.filter((value) => value !== price); const nextStructured = event.target.checked ? [...(teaching.qualifyingLevels ?? []).filter((item) => item.levelId !== level.id), { levelId: level.id, levelType, valueAtInteraction: price, sourceTimestamp: new Date(levelCandle?.openTime ?? Date.now()).toISOString(), rangeLow: levelType === "dynamic_indicator" ? null : level.rangeLow ?? null, rangeHigh: levelType === "dynamic_indicator" ? null : level.rangeHigh ?? null }] : (teaching.qualifyingLevels ?? []).filter((item) => item.levelId !== level.id); updateTeaching({ pullbackLevels: nextLevels, qualifyingLevels: nextStructured, ...(event.target.checked && !teaching.qualifyingLevelId ? { qualifyingLevelId: level.id, qualifyingLevelRangeLow: level.rangeLow, qualifyingLevelRangeHigh: level.rangeHigh } : {}) }); }} /><span><span className="block font-bold">{level.label}</span><span className="mono text-muted-foreground">{formatPriceAxisValue(price)} · {level.id}</span>{!contained && <span className="block text-[9px] text-negative">Outside {levelToleranceTicks}-tick zone at L</span>}</span></label>; })}</div><p className="mt-1.5 text-[10px] leading-4 text-muted-foreground">Levels intersect L within the configured {levelToleranceTicks}-tick MES proximity zone. Dynamic VWAP and EMA values are captured from the indicator point at L; fixed levels retain their causal ranges.</p></fieldset>
