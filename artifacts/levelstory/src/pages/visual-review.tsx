...
const STRATEGY_TABS: Array<{ id: StrategyId; label: string }> = [
  ...
  { id: "EARLY_ORB_MOMENTUM_CONTINUATION", label: "Early ORB Momentum Continuation" },
...
function freshnessReasonLabel(reason: string): string {
  return ({
    ...
    cache_key_mismatch: "source unavailable or request/source cache identity changed",
...
export default function VisualReview() {
  ...
  const handleVisualReviewTabKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    ...
    if (nextTab) {
      setActiveVisualReviewTab(nextTab.id);
      window.requestAnimationFrame(() => document.getElementById(`visual-review-tab-${nextTab.id}`)?.focus());
    }
  };

  const handleReviewDetailTabKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight" && event.key !== "Home" && event.key !== "End") return;
    event.preventDefault();
    const currentIndex = REVIEW_DETAIL_TABS.findIndex((tab) => tab.id === activeReviewDetailTab);
    const nextIndex = event.key === "Home"
      ? 0
      : event.key === "End"
        ? REVIEW_DETAIL_TABS.length - 1
        : (currentIndex + (event.key === "ArrowRight" ? 1 : -1) + REVIEW_DETAIL_TABS.length) % REVIEW_DETAIL_TABS.length;
    const nextTab = REVIEW_DETAIL_TABS[nextIndex];
    if (nextTab) {
      setActiveReviewDetailTab(nextTab.id);
      window.requestAnimationFrame(() => document.getElementById(`review-detail-tab-${nextTab.id}`)?.focus());
    }
  };
         ...
         <div className="mx-auto flex max-w-[1560px] flex-col">
           ...
           {activeVisualReviewTab === "generate" && data && activeSnapshot && <section id="visual-review-panel-generate-evidence" aria-label="Generated replay chart evidence" className="order-2 space-y-5">
                      ...
                      <div role="tablist" aria-label="Selected trade supporting information" className="grid gap-1 sm:grid-cols-4">
                        {REVIEW_DETAIL_TABS.map((tab) => {
                          ...
                          return <button
                            ...
                            onKeyDown={handleReviewDetailTabKeyDown}
            ...
            {activeVisualReviewTab === "generate" && <section id="visual-review-panel-generate" role="tabpanel" aria-labelledby="visual-review-tab-generate" tabIndex={0} className="order-1 space-y-5">
              <div className={data || generationActive ? "" : "grid items-start gap-5 xl:grid-cols-[minmax(280px,.7fr)_minmax(0,1.3fr)]"}>
...
function SnapshotHeaderContent({ snapshot, request, index, total, onPrevious, onNext, includeProvenance = true }: { snapshot: VisualValidationSnapshot; request: VisualValidationRequest; index: number; total: number; onPrevious: () => void; onNext: () => void; includeProvenance?: boolean }) {
  ...
  return <div className="border-t border-border bg-muted/20" data-testid="historical-review-sample">
    ...
    {includeProvenance && <SnapshotCursorDetails snapshot={snapshot} request={request} />}
     {includeProvenance && <SnapshotProvenance snapshot={snapshot} />}
  </div>;
}

function SnapshotCursorDetails({ snapshot, request }: { snapshot: VisualValidationSnapshot; request: VisualValidationRequest }) {
  return <div className="grid gap-px border-t border-border bg-border sm:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="grid gap-2">
       ...
       {needsTeaching && <div className="space-y-3 border border-accent/35 bg-accent/5 p-4" data-testid="teaching-example-form">
         ...
         {teaching && <div className="grid gap-3 sm:grid-cols-2">
           ...
           <Field label="Confirmation buffer"><select className="field mono" value={teaching.entryBufferTicks} onChange={(event) => updateTeaching({ entryBufferTicks: 4 })}><option value={4}>4 MES ticks · 1.00 point</option></select></Field>
