      ...
      <div className="cockpit-grid min-h-[calc(100dvh-62px)] px-4 py-6 sm:px-7 lg:px-9 lg:py-8">
        <div className="mx-auto max-w-[1560px]">
          ...
          <div className="mb-5 grid gap-5 xl:grid-cols-[minmax(280px,.7fr)_minmax(0,1.3fr)]">
            ...
            {generationActive ? <div className="mt-5"><ShadowAccountReplayPanel reviewSetId={replayReviewSetId} generationActive onRegenerateFresh={regenerateFreshReviewSet} query={shadowReplayQuery} startingBalance={startingBalance} setStartingBalance={setStartingBalance} contractsPerTrade={contractsPerTrade} setContractsPerTrade={setContractsPerTrade} /></div> : setQuery.isLoading && !data ? <Panel><QuerySkeleton rows={6} /></Panel> : setQuery.isError && !data ? apiErrorStatus(setQuery.error) === 404 && reviewSetId ? (
             <Panel accent>
            ...
            <div className="space-y-5">
              ...
              <ShadowAccountReplayPanel reviewSetId={replayReviewSetId} onRegenerateFresh={regenerateFreshReviewSet} query={shadowReplayQuery} startingBalance={startingBalance} setStartingBalance={setStartingBalance} contractsPerTrade={contractsPerTrade} setContractsPerTrade={setContractsPerTrade} />
            </div>
          ) : (
            <>
                  ...
                  <div className="visual-review-chart-column min-w-0 space-y-5">
                    ...
                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(280px,.42fr)]">
                      ...
                      <div className="space-y-5">
                        ...
                        <ShadowAccountReplayPanel reviewSetId={data.reviewSetId} onRegenerateFresh={regenerateFreshReviewSet} query={shadowReplayQuery} startingBalance={startingBalance} setStartingBalance={setStartingBalance} contractsPerTrade={contractsPerTrade} setContractsPerTrade={setContractsPerTrade} />
              ...
              ) : <div className="space-y-5"><UnavailableWorkspace coverage={coverage} source={data.source} /><ShadowAccountReplayPanel reviewSetId={data.reviewSetId} onRegenerateFresh={regenerateFreshReviewSet} query={shadowReplayQuery} startingBalance={startingBalance} setStartingBalance={setStartingBalance} contractsPerTrade={contractsPerTrade} setContractsPerTrade={setContractsPerTrade} /></div>}
            </>
          )}
        </div>
      </div>
  ...
  const stateContent = () => {
    ...
    if (replay.enteredTrades === 0) {
      return <div className="border border-dashed border-border bg-muted/20 px-4 py-5" data-testid="shadow-replay-zero-trades">
        <div className="flex items-center gap-2 text-sm font-bold"><Info size={15} className="text-accent" />No entered trades in this review set.</div>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">Candidate detection ran, but no risk-approved entries changed the shadow account balance. There is no equity curve or ledger to inspect.</p>
      </div>;
    }
    ...
    <div className="border-t border-border px-5 py-4 sm:px-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-xl text-xs leading-5 text-muted-foreground">
          <span className="font-bold text-foreground">Dummy money only.</span> Replays the currently selected Visual Review set with fixed contract sizing. No compounding, broker, paper, or live action is available from this panel.
        </div>
...
function ShadowReplayResults({ replay, metric }: { replay: ShadowAccountReplay; metric: (label: string, value: string, detail?: string, tone?: string) => ReactNode }) {
  return <div className="space-y-4" data-testid="shadow-replay-results">
    ...
    <div className="grid gap-px border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
      ...
      {metric("Entered trades", formatAccountNumber(replay.enteredTrades, 0), `${replay.closedTrades} closed · ${replay.openTrades} open`)}
      {metric("Wins / losses", `${replay.wins} / ${replay.losses}`, `${replay.openTrades} open`)}
  ...
  return <div className="border border-border bg-muted/12" data-testid="shadow-equity-curve">
    ...
    <div className="overflow-x-auto px-2 py-3">
      <svg viewBox={`0 0 ${width} ${height}`} className="h-[220px] min-w-[560px] w-full" role="img" aria-label="Shadow account realized equity curve">
        ...
        {curve.length > 0 && <><text x={pad.left} y={height - 9} fill="hsl(var(--muted-foreground))" fontSize="9" fontFamily="DM Mono">T1</text><text x={width - pad.right} y={height - 9} textAnchor="end" fill="hsl(var(--muted-foreground))" fontSize="9" fontFamily="DM Mono">T{curve[curve.length - 1].tradeNumber}</text></>}
      </svg>
    </div>
  </div>;
}
...
function ShadowLedger({ replay }: { replay: ShadowAccountReplay }) {
  return <div className="border border-border" data-testid="shadow-trade-ledger">
    ...
    <div className="overflow-x-auto">
        ...
        <thead className="bg-muted/35 mono text-[9px] uppercase tracking-[.07em] text-muted-foreground"><tr>{["Trade", "Date/time ET", "Contract", "Edge / strategy", "Dir", "Entry", "Exit", "Exit reason", "Net P/L", "Balance", "Confluences", "Period", "Status"].map((heading) => <th key={heading} className="whitespace-nowrap border-b border-border px-3 py-2.5 font-medium">{heading}</th>)}</tr></thead>
        <tbody>
          {replay.ledger.map((trade) => <tr key={`${trade.tradeNumber}-${trade.candidateId}`} className="border-b border-border/70 last:border-b-0 hover:bg-muted/20">
            ...
            <td className="px-3 py-2.5"><span className={`inline-flex border px-1.5 py-1 text-[9px] font-bold uppercase ${trade.status === "open" ? "border-border text-muted-foreground" : trade.netPnl !== null && trade.netPnl < 0 ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-[hsl(var(--positive)/.3)] bg-[hsl(var(--positive)/.1)] text-[hsl(var(--positive))]"}`}>{trade.status}</span></td>
          </tr>)}
        </tbody>
      </table>
    </div>
