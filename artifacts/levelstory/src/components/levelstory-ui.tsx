...
export function MiniCandleChart({ candles, ntz, levels = [] }: { candles: Candle[]; ntz?: NtzState; levels?: Array<{ name: string; price: number }> }) {
  ...
  const ntzBarCount = ntzStartIndex >= 0 && ntzEndIndex >= 0 ? ntzEndIndex - ntzStartIndex : 0;
   return <div className="w-full overflow-x-auto px-4 pb-3" data-testid="chart-candles"><svg viewBox={`0 0 ${width} ${height}`} className="h-[248px] w-full min-w-[600px]" role="img" aria-label="Calculated five minute candle chart with NTZ and level overlays">
