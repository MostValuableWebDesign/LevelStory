...
type AnalysisResponse = {
  ...
  candidate: {
    ...
    targetPlan?: {
      ...
      selectedTargetLevel: { id: string; type: string; price: number } | null;
      skippedLevels: Array<{
        id: string;
        price: number;
        reason: string;
    ...
    <div className="space-y-5 border-t border-border p-5 sm:p-6">
      ...
      {analysis && <div className="space-y-4 border-t border-border pt-5" data-testid="uploaded-chart-result">
        ...
        <div><div className="eyebrow mb-2 text-muted-foreground">Structured rule results</div><div className="grid gap-2 sm:grid-cols-2">{(extraction?.rules ?? []).map((rule) => <div key={rule.name} className="border border-border bg-card p-3 text-[11px]"><div className="flex items-start justify-between gap-2"><span className="font-bold">{rule.name}</span><span className={`mono text-[9px] font-bold uppercase ${rule.status === "pass" ? "text-[hsl(var(--positive))]" : rule.status === "fail" ? "text-destructive" : "text-muted-foreground"}`}>{rule.status}</span></div><div className="mt-1 text-muted-foreground">{rule.explanation}</div>{rule.value && <div className="mono mt-2 text-[10px]">{rule.value}</div>}</div>)}</div></div>
         {candidate && <div className="border border-[hsl(var(--positive)/.3)] bg-[hsl(var(--positive)/.06)] p-4"><div className="eyebrow text-[hsl(var(--positive))]">Candidate / Shadow Mode</div><div className="mt-2 grid gap-3 text-[11px] sm:grid-cols-4"><Metric label="Direction" value={candidate.direction} /><Metric label="Entry" value={candidate.entryTriggerPrice.toFixed(2)} /><Metric label="Stop" value={candidate.stopPrice.toFixed(2)} /><Metric label="Target" value={candidate.targetPrice?.toFixed(2) ?? "Not visible"} /><Metric label="Contracts" value={String(candidate.contracts)} /><Metric label="Modeled risk" value={`$${candidate.riskDollars.toFixed(2)}`} /><Metric label="Grade" value={candidate.setupGrade} /><Metric label="Outcome" value={candidate.outcome.replaceAll("_", " ")} /></div>{candidate.targetPlan && <div className="mt-4 border-t border-[hsl(var(--positive)/.2)] pt-3"><div className="eyebrow text-muted-foreground">Shared target plan · {candidate.targetPlan.targetPlanVersion}</div><div className="mt-2 grid gap-3 text-[11px] sm:grid-cols-4"><Metric label="Raw level" value={candidate.targetPlan.selectedLevelPrice?.toFixed(2) ?? "1R fallback"} /><Metric label="Executable target" value={candidate.targetPlan.targetPrice?.toFixed(2) ?? "Unavailable"} /><Metric label="Near-side buffer" value={`${candidate.targetPlan.targetBufferTicks} ticks · ${candidate.targetPlan.targetBufferPoints.toFixed(2)} points`} /><Metric label="Distance / R" value={`${candidate.targetPlan.targetDistanceTicks ?? "—"} ticks · ${candidate.targetPlan.targetR == null ? "—" : `${candidate.targetPlan.targetR.toFixed(2)}R`}`} /></div><div className="mt-2 text-[10px] text-muted-foreground">Provenance: {candidate.targetProvenance?.replaceAll("_", " ") ?? "shared planner"}{candidate.targetPlan.fallbackUsed ? " · no buffered visible level qualified, so the target is exactly 1R." : candidate.targetPlan.selectedTargetLevel ? ` · ${candidate.targetPlan.selectedTargetLevel.id} (${candidate.targetPlan.selectedTargetLevel.type})` : ""}</div>{candidate.targetPlan.skippedLevels.length > 0 && <div className="mt-3 space-y-1 border-t border-[hsl(var(--positive)/.15)] pt-2 text-[10px]"><div className="font-bold">Skipped executable levels</div>{candidate.targetPlan.skippedLevels.map((level) => <div key={`${level.id}-${level.reason}`} className="flex flex-wrap justify-between gap-x-3 gap-y-1 text-muted-foreground"><span>{level.id}: {level.reason}</span><span className="mono">raw {level.price.toFixed(2)} → {level.executableTargetPrice?.toFixed(2) ?? "—"}{level.executableTargetR == null ? "" : ` · ${level.executableTargetR.toFixed(2)}R`}</span></div>)}</div>}</div>}</div>}
