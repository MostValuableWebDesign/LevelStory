...
test("visual review presentation retains human-only shading and semantic level colors", () => {
  ...
  assert.match(page, /data-testid="example-sample-summary"/);
  assert.match(page, /Example 01 \/ 01/);
  assert.match(page, /selectedExample\.categoryLabel/);
  ...
  assert.doesNotMatch(page, /data-testid="formula-development-sample"/);
