...
test("visual review presentation retains human-only shading and semantic level colors", () => {
  ...
  assert.match(page, /formatPriceAxisValue\(annotation\.price!\)/);
    ...
    assert.match(page, /const chartLabel = chartLevelLabel\(annotation\)/);
    ...
    assert.match(page, /primaryLevels\.filter\(\(annotation\) => isDynamicIndicatorAnnotation/);
    assert.match(page, /chartLabel && <text/);
