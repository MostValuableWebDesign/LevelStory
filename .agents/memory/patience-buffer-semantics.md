...
The patience engine treats a raw facing-extreme break as observation, not entry. A trigger is valid only after the same immediate candle reaches the complete governed four-tick confirmation buffer; intrabar ordering evidence may resolve ambiguity but cannot waive that buffer. The opposite patience extreme invalidates the setup, and a closed immediate candle that never reaches the buffer expires it.
