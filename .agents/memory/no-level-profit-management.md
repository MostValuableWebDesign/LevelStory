...
When a candidate has no eligible farther key-level target, calculate 1R from the modeled entry fill to the operative initial stop. Do not activate structure trailing before price reaches +1R. For one contract, keep the full position and trail it after +1R; for multiple contracts, exit one contract at +1R and trail the remaining runner. A structure stop is 8 MES ticks beyond the most recent completed five-minute swing in the adverse direction, and it may only advance.
