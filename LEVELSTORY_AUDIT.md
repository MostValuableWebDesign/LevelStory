...
| Unclear trend cannot qualify | Functional | `trendEvidence()` and Phase 6 setup rules; snapshot decision; lockout/causal tests | No separately rendered neutral “cannot qualify” fixture in the frontend smoke suite | Phase 1 |
...
| Completed close outside NTZ gate | Functional | Phase 4 breakout state and Phase 6 setup rules; snapshot regular/premarket tests | No user-controlled cursor to test the exact transition interactively | Phase 1 |
...
| Completed ORB close in trend direction | Functional | Phase 4 breakout quality/state evaluation; regular snapshot contract test | No separate API endpoint for evaluating arbitrary cursor points | Phase 1 |
| Mandatory subsequent breakout pullback | Partially functional | Phase 4 `analyzePullback()` and Phase 6 setup evaluation | Pullback sequence is represented by the final candle only; no persisted event or detailed pullback window | Phase 1 |
| Pullback reaches ORB edge plus independent confluence | Partially functional | Phase 4 `analyzePullback()` and Phase 6 setup evaluation | Confluence evidence is reduced to a boolean; the API does not identify the exact qualifying confluence set | Phase 1 |
...
| Fibonacci is confluence only | Functional | Phase 4 Fibonacci and pullback analysis; no standalone Fibonacci signal | The API does not explicitly label Fibonacci’s role as confluence-only | Phase 1 |
| Patience candle and closed-candle requirement | Partially functional | Phase 5 `phase5PatienceAnalysis()`; Phase 6 setup rules; tests | Current snapshot does not expose a highlighted previous/patience candle pair or a full post-pullback event chain | Phase 1 |
...
| Full qualification chain | Functional | Phase 4–8 phased evaluator; API `decision`; dashboard no longer uses local qualification toggles | Some rule calculations still use the latest candle as the pullback/patience candidate | Phase 1 |
